...
CBScript * CBNewScriptFromReference(CBByteArray * program, int offset, int len){
	return CBNewByteArraySubReference(program, offset, len);
}

CBScript * CBNewScriptOfSize(int size){
...
CBScript * CBNewScriptMultisigOutput(unsigned char ** pubKeys, int m, int n){
	CBScript * self = malloc(sizeof(*self));
	CBGetObject(self)->free = CBFreeByteArray;
...
CBScript * CBNewScriptPubKeyHashOutput(unsigned char * pubKeyHash){
	CBScript * self = malloc(sizeof(*self));
	CBGetObject(self)->free = CBFreeByteArray;
...
CBScript * CBNewScriptPubKeyOutput(unsigned char * pubKey){
	CBScript * self = malloc(sizeof(*self));
	CBGetObject(self)->free = CBFreeByteArray;
...
CBScript * CBNewScriptWithData(unsigned char * data, int size){
	return CBNewByteArrayWithData(data, size);
}

CBScript * CBNewScriptWithDataCopy(unsigned char * data, int size){
	...
	for (char * cursor = string; *cursor != '\0';) {
		switch (*cursor) {
			case 'O':
				...
				space = false;
				line = false;
				unsigned char * temp = realloc(data, dataLast + 1);
				data = temp;
			...
			case '0':
				...
				if (cursor[1] == 'x') {
					...
					if (spaceFind == NULL)
						if (lineFind == NULL)
							diff = (int)strlen(cursor);
						else
							diff = (int)(lineFind - cursor);
					else
						if (lineFind == NULL)
							diff = (int)(spaceFind - cursor);
					...
					if (num < 76) {
						unsigned char * temp = realloc(data, dataLast + 1 + num);
						data = temp;
						data[dataLast] = num;
						dataLast++;
					}else if (num < 256) {
						unsigned char * temp = realloc(data, dataLast + 2 + num);
						data = temp;
						...
						unsigned char * temp = realloc(data, dataLast + 3 + num);
						data = temp;
						...
						unsigned char * temp = realloc(data, dataLast + 5 + num);
						data = temp;
...
void CBInitScriptMultisigOutput(CBScript * self, unsigned char ** pubKeys, int m, int n) {
	...
	for (int x = 0; x < n; x++, cursor += CB_PUBKEY_SIZE + 1) {
	...
	CBByteArraySetByte(self, cursor + 1, CB_SCRIPT_OP_CHECKMULTISIG);
...
void CBInitScriptPubKeyHashOutput(CBScript * self, unsigned char * pubKeyHash){
...
void CBInitScriptPubKeyOutput(CBScript * self, unsigned char * pubKey){
...
CBScriptExecuteReturn CBScriptExecute(CBScript * self, CBScriptStack * stack, bool (*getHashForSig)(void *, CBByteArray *, int, CBSignType, unsigned char *), void * transaction, int inputIndex, bool p2sh){
	...
	int skipIfElseBlock = 0xffff; // Skips all instructions on or over this if/else level.
	int ifElseSize = 0; // Amount of if/else block levels
	int beginSubScript = 0;
	int cursor = 0;
	...
	for (int opCount = 0; self->length - cursor > 0;) {
		...
		if (ifElseSize >= skipIfElseBlock) { // Skip when "ifElseSize" level is over or at "skipIfElseBlock"
			...
			if (! byte) {
				...
				// Check size
				if ((unsigned int)(self->length - cursor) < byte)
				...
				if (byte == CB_SCRIPT_OP_PICK) {
					...
					for (int x = 0; x < i; x++) // Move other elements down
				...
				// Convert enum to unsigned char to get integer promotion
				unsigned char cbyte = byte;

				if (stack->length < cbyte - CB_SCRIPT_OP_2DUP + 2)
					return CB_SCRIPT_INVALID; // Stack needs more elements.

				for (int x = 0; x < cbyte - CB_SCRIPT_OP_2DUP + 2; x++) {
				...
				if (i1.length != i2.length)
					ok = false;
				else for (int x = 0; x < i1.length; x++)
				...
				if ((! res && byte == CB_SCRIPT_OP_NOT) || (res && byte == CB_SCRIPT_OP_0NOTEQUAL)) {
					item.length = 1;
					unsigned char * temp = realloc(item.data, 1);
					item.data = temp;
					item.data[0] = 1;
				}else{
				...
				free(i2.data); // No longer need i2.
				i1.length = 1;
				unsigned char * temp = realloc(i1.data, 1);
				i1.data = temp;
				...
				int subScriptLen = self->length - beginSubScript;
				unsigned char * subScript = malloc(subScriptLen);
				unsigned char * sourceScript = CBByteArrayGetData(self);
				unsigned char * subScriptCopyPointer = subScript;
				unsigned char * lastSeparator = sourceScript + beginSubScript;
				unsigned char * end = sourceScript + self->length;
				unsigned char * ptr = lastSeparator;
				...
				for (;;) {
					if (ptr >= end) {
						...
						int len = (int)(ptr - lastSeparator); // Do not include code separator.
				...
				if (byte == CB_SCRIPT_OP_CHECKSIG
					...
					int sig = 3 + numKeys; // To first signature.
					int key = 2; // To first key.
					...
					for (int x = 0; x < removeItemsNum; x++)
						CBScriptStackRemoveItem(stack);
				}
				if (byte == CB_SCRIPT_OP_CHECKSIG
					|| byte == CB_SCRIPT_OP_CHECKMULTISIG) {
...
int CBScriptGetLengthOfPushOp(int dataLen){
...
int CBScriptGetPushAmount(CBScript * self, int * offset){
...
int CBScriptGetSigOpCount(CBScript * self, bool inP2SH){
	int sigOps = 0;
...
bool CBScriptIsMultisig(CBScript * self){
	...
	if (CBByteArrayGetByte(self, self->length - 1) != CB_SCRIPT_OP_CHECKMULTISIG)
		return false;
	int sigNum = CBScriptOpGetNumber(CBByteArrayGetByte(self, 0));
	if (sigNum == CB_NOT_A_NUMBER_OP || sigNum == 0)
		return false;
	int pubKeyNum = CBScriptOpGetNumber(CBByteArrayGetByte(self, self->length - 2));
	if (pubKeyNum == CB_NOT_A_NUMBER_OP || pubKeyNum < sigNum)
		return false;
	// Check public keys
	int cursor = 1;
	for (int x = 0; x < pubKeyNum; x++) {
		int pushAmount = CBScriptGetPushAmount(self, &cursor);
...
int CBScriptIsPushOnly(CBScript * self){
	int x = 0;
	int num = 0;
	for (; x < self->length;) { 
...
int CBScriptOpGetNumber(CBScriptOp op){
...
void CBSubScriptRemoveSignature(unsigned char * subScript, int * subScriptLen, CBScriptStackItem signature){
	if (signature.data == NULL) return; // Signature zero
	unsigned char * ptr = subScript;
	unsigned char * end = subScript + *subScriptLen;
	for (;ptr < end;) {
		if (*ptr && *ptr < 79) { // Push
...
CBScriptStackItem CBScriptStackCopyItem(CBScriptStack * stack, int fromTop){
...
int CBScriptStringMaxSize(CBScript * self){
	int size = 0, amount;
	for (int x = 0; x < self->length;) {
...
void CBScriptToString(CBScript * self, char * output){
	...
	for (int x = 0; x < self->length;) {
		...
		if (op == CB_SCRIPT_OP_0) {
			...
			// Push opperation
			int amount = CBScriptGetPushAmount(self, &x);
			strcpy(output, "0x");
			output += 2;
			x -= amount;
			for (int y = 0; y < amount; y++, x++, output += 2)
...
void CBScriptWritePushOp(CBScript * self, int offset, unsigned char * data, int dataLen){
	int len = CBScriptGetLengthOfPushOp(dataLen);
	if (len == 1)
...
CBScriptStackItem CBInt64ToScriptStackItem(CBScriptStackItem item, int64_t i){
	...
	unsigned char * temp = realloc(item.data, item.length);
	item.data = temp;
	// Add data
	for (int x = 0; x < item.length; x++)
		if (x == 8)
