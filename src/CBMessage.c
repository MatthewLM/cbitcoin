...
void CBMessagePrepareBytes(CBMessage * message, int length){
	...
	if (message->bytes) {
		...
		if (message->bytes->length < length) {
			
			CBReleaseObject(message->bytes);
			message->bytes = NULL;
			
		}else if (message->bytes->length > length)
			message->bytes->length = length;
		
	}
	
	if (! message->bytes)
		message->bytes = CBNewByteArrayOfSize(length);
	
	message->serialised = false;
