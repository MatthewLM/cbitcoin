...
CBTransactionInput * CBNewTransactionInput(CBScript * script, int sequence, CBByteArray * prevOutHash, int prevOutIndex) {
...
CBTransactionInput * CBNewTransactionInputTakeScriptAndHash(CBScript * script, int sequence, CBByteArray * prevOutHash, int prevOutIndex) {
...
void CBInitTransactionInput(CBTransactionInput * self, CBScript * script, int sequence, CBByteArray * prevOutHash, int prevOutIndex) {
...
void CBInitTransactionInputTakeScriptAndHash(CBTransactionInput * self, CBScript * script, int sequence, CBByteArray * prevOutHash, int prevOutIndex) {
...
int CBTransactionInputCalculateLength(CBTransactionInput * self) {
	
	return 40 + CBVarIntSizeOf(self->scriptObject->length) + self->scriptObject->length;
	
}

int CBTransactionInputDeserialise(CBTransactionInput * self) {
	...
	self->scriptObject = CBNewScriptFromReference(bytes, 36 + scriptLen.size, (int) scriptLen.val);
	self->sequence = CBByteArrayReadInt32(bytes, (int) (36 + scriptLen.size + scriptLen.val));
...
void CBTransactionInputPrepareBytes(CBTransactionInput * self) {
	...
	CBMessagePrepareBytes(CBGetMessage(self), CBTransactionInputCalculateLength(self));
	
}

int CBTransactionInputSerialise(CBTransactionInput * self) {
	...
	int reqLen = 40 + scriptLen.size + CBGetByteArray(self->scriptObject)->length;
	if (bytes->length < reqLen) {
	...
	CBByteArraySetVarInt(bytes, 36, scriptLen);
