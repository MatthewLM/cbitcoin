...
void CBBigIntAlloc(CBBigInt * bi, int allocLen){
...
void CBBigIntEqualsDivisionBy58(CBBigInt * a, unsigned char * ans){
...
void CBBigIntEqualsMultiplicationByUInt8(CBBigInt * a, int b){
	if (! b) {
...
void CBBigIntEqualsSubtractionByUInt8(CBBigInt * a, int b){
	int carry = b;
	int x = 0;
...
void CBBigIntFromPowUInt8(CBBigInt * bi, int a, int b){
	bi->length = 1;
	bi->data[0] = 1;
	for (int x = 0; x < b; x++)
		CBBigIntEqualsMultiplicationByUInt8(bi, a);
}
int CBBigIntModuloWith58(CBBigInt * a){
	// Use method presented here: http://stackoverflow.com/a/10441333/238411
	int result = 0; // Prevent overflow in calculations
	for(int x = a->length - 1;; x--){
		result *= (256 % 58);
		result %= 58;
		result += a->data[x] % 58;
		result %= 58;
...
void CBBigIntRealloc(CBBigInt * bi, int allocLen){
	if (bi->allocLen < allocLen) {
		unsigned char * temp = realloc(bi->data, allocLen);
		bi->data = temp;
		bi->allocLen = allocLen;
	}
}
