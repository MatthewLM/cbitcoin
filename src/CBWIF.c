...
CBWIF * CBNewWIFFromPrivateKey(unsigned char * privKey, bool useCompression, CBBase58Prefix prefix, bool cacheString){
...
void CBInitWIFFromPrivateKey(CBWIF * self, unsigned char * privKey, bool useCompression, CBBase58Prefix prefix, bool cacheString){
	// Build WIF and then complete intitialisation with CBChecksumBytes
	unsigned char * data = malloc(37 + useCompression); // 1 byte for 0x80, 32 key bytes, 4 checksum bytes. Extra byte for compression.
	// Set prefix
	data[0] = prefix;
	// Move key
...
void CBWIFGetPrivateKey(CBWIF * self, unsigned char * privKey){
