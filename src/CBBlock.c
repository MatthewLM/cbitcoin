...
CBBlock * CBNewBlock(void (*logError)(char *,...)){
	...
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewBlock\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeBlock;
	if(CBInitBlock(self,logError))
		return self;
	free(self);
	return NULL;
}
CBBlock * CBNewBlockFromData(CBByteArray * data,void (*logError)(char *,...)){
	CBBlock * self = malloc(sizeof(*self));
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewBlockFromData\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeBlock;
	if(CBInitBlockFromData(self,data,logError))
		return self;
	free(self);
	return NULL;
}
CBBlock * CBNewBlockGenesis(void (*logError)(char *,...)){
	CBBlock * self = malloc(sizeof(*self));
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewBlockGenesis\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeBlock;
	if(CBInitBlockGenesis(self,logError))
		return self;
	free(self);
	return NULL;
}
...
uint32_t CBBlockDeserialise(CBBlock * self,bool transactions){
	...
	if (NOT bytes) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBBlock with no bytes.");
		return 0;
	}
	if (bytes->length < 82) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBBlock with less than 82 bytes. Minimum for header (With null byte).");
		return 0;
	}
	self->version = CBByteArrayReadInt32(bytes, 0);
	self->prevBlockHash = CBByteArraySubReference(bytes, 4, 32);
	if (NOT self->prevBlockHash){
		CBGetMessage(self)->logError("Cannot create the previous block hash CBByteArray in CBBlockDeserialise.");
		return 0;
	}
	self->merkleRoot = CBByteArraySubReference(bytes, 36, 32);
	if (NOT self->merkleRoot){
		CBGetMessage(self)->logError("Cannot create the merkle root CBByteArray in CBBlockDeserialise.");
		return 0;
	}
	...
	if (transactions && firstByte) {
		// More to come
		if (bytes->length < 89) {
			CBGetMessage(self)->logError("Attempting to deserialise a CBBlock with a non-zero varint with less than 89 bytes.");
			return 0;
		}
		CBVarInt transactionNumVarInt = CBVarIntDecode(bytes, 80);
		if (transactionNumVarInt.val*60 > bytes->length - 81) {
			CBGetMessage(self)->logError("Attempting to deserialise a CBBlock with too many transactions for the byte data length.");
			return 0;
		}
		...
		if (NOT self->transactionNum) {
			CBGetMessage(self)->logError("Cannot allocate %i bytes of memory in CBBlockDeserialise\n",sizeof(*self->transactions) * self->transactionNum);
			return 0;
		}
		...
		for (uint16_t x = 0; x < self->transactionNum; x++) {
			...
			if (NOT data) {
				CBGetMessage(self)->logError("Could not create a new CBByteArray in CBBlockDeserialise for the transaction number %u.",x);
			}
			CBTransaction * transaction = CBNewTransactionFromData(data, CBGetMessage(self)->logError);
			if (NOT transaction){
				CBGetMessage(self)->logError("Could not create a new CBTransaction in CBBlockDeserialise for the transaction number %u.",x);
				CBReleaseObject(data);
				return 0;
			}
			uint32_t len = CBTransactionDeserialise(transaction);
			if (NOT len){
				CBGetMessage(self)->logError("CBBlock cannot be deserialised because of an error with the transaction number %u.",x);
				CBReleaseObject(data);
				return 0;
			}
		...
		if (bytes->length < 80 + x + 1) {
			CBGetMessage(self)->logError("Attempting to deserialise a CBBlock header with not enough space to cover the var int.");
			return 0;
		}
		...
		if (CBByteArrayGetByte(bytes, 80 + x) != 0) {
			CBGetMessage(self)->logError("Attempting to deserialise a CBBlock header with a final byte which is not null. This is not what it is supposed to be but you already knew that right?");
			return 0;
		}
...
uint32_t CBBlockSerialise(CBBlock * self, bool transactions, bool force){
	CBByteArray * bytes = CBGetMessage(self)->bytes;
	if (NOT bytes) {
		CBGetMessage(self)->logError("Attempting to serialise a CBBlock with no bytes.");
		return 0;
	}
	...
	if (bytes->length < cursor + 1) {
		CBGetMessage(self)->logError("Attempting to serialise a CBBlock with less bytes than required for the header, transaction number var int and at least a null byte. %i < %i\n",bytes->length, cursor);
		return 0;
	}
	// Do header
		...
		for (uint32_t x = 0; x < self->transactionNum; x++) {
			...
			if (NOT CBGetMessage(self->transactions[x])->serialised || force) {
				...
				if (NOT CBGetMessage(self->transactions[x])->bytes) {
					CBGetMessage(self)->logError("Cannot create a new CBByteArray sub reference in CBBlockSerialise for the transaction number %u",x);
					return 0;
				}
				uint32_t len = CBTransactionSerialise(self->transactions[x], force);
				if (NOT len) {
					CBGetMessage(self)->logError("CBBlock cannot be serialised because of an error with the transaction number %u.",x);
					return 0;
				}
