...
uint32_t CBBlockDeserialise(CBBlock * self, bool transactions) {
	...
	if (transactions && txNumVI.val) {
		...
		self->transactionNum = (uint32_t)txNumVI.val;
		self->transactions = malloc(sizeof(*self->transactions) * self->transactionNum);
		
		uint32_t cursor = 80 + txNumVI.size;
	...
	if (transactions) {
		...
		for (uint32_t x = 0; x < self->transactionNum; x++) {
			...
			if (! CBGetMessage(self->transactions[x])->serialised // Serailise if not serialised yet.
				...
				for (uint32_t y = 0; y < self->transactions[x]->inputNum; y++) {
				...
				for (uint32_t y = 0; y < self->transactions[x]->outputNum; y++) {
