...
CBByteArray * CBNewByteArrayFromString(char * string, bool terminator, void (*logError)(char *,...)){
	CBByteArray * self = malloc(sizeof(*self));
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewByteArrayFromString\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeByteArray;
	if(CBInitByteArrayFromString(self,string,terminator,logError))
		return self;
	free(self);
	return NULL;
}
...
bool CBInitByteArrayOfSize(CBByteArray * self,uint32_t size,void (*logError)(char *,...)){
	if (NOT CBInitObject(CBGetObject(self)))
		return false;
	self->logError = logError;
	self->length = size;
	...
	self->sharedData = malloc(sizeof(*self->sharedData));
	if (NOT self->sharedData) {
		logError("Cannot allocate %i bytes of memory in CBInitByteArrayOfSize for the sharedData structure.\n",sizeof(*self->sharedData));
		return false;
	}
	self->sharedData->references = 1;
	self->sharedData->data = malloc(size);
	if (NOT self->sharedData->data) {
		logError("Cannot allocate %i bytes of memory in CBInitByteArrayOfSize for the shared data.\n",size);
		return false;
	}
	return true;
}
...
bool CBInitByteArrayWithDataCopy(CBByteArray * self,uint8_t * data,uint32_t size,void (*logError)(char *,...)){
	if (NOT CBInitObject(CBGetObject(self)))
		return false;
	self->logError = logError;
	self->sharedData = malloc(sizeof(*self->sharedData));
	if (NOT self->sharedData) {
		logError("Cannot allocate %i bytes of memory in CBInitByteArrayWithDataCopy for the sharedData structure.\n",sizeof(*self->sharedData));
		return false;
	}
	self->sharedData->data = malloc(size);
	if (NOT self->sharedData->data) {
		logError("Cannot allocate %i bytes of memory in CBInitByteArrayWithDataCopy for the shared data.\n",size);
		return false;
	}
...
CBByteArray * CBByteArrayCopy(CBByteArray * self){
	CBByteArray * new = CBNewByteArrayOfSize(self->length,self->logError);
	if (NOT new)
		return NULL;
...
CBByteArray * CBByteArraySubCopy(CBByteArray * self,uint32_t offset,uint32_t length){
	CBByteArray * new = CBNewByteArrayOfSize(length,self->logError);
	if (NOT new)
		return NULL;
