...
CBGetBlocks * CBNewGetBlocks(int version, CBChainDescriptor * chainDescriptor, CBByteArray * stopAtHash) {
...
void CBInitGetBlocks(CBGetBlocks * self, int version, CBChainDescriptor * chainDescriptor, CBByteArray * stopAtHash) {
...
int CBGetBlocksCalculateLength(CBGetBlocks * self) {
	
	return 36 + self->chainDescriptor->hashNum * 32 + CBVarIntSizeOf(self->chainDescriptor->hashNum);
	
}

int CBGetBlocksDeserialise(CBGetBlocks * self) {
	...
	int len = CBChainDescriptorDeserialise(self->chainDescriptor);
...
int CBGetBlocksSerialise(CBGetBlocks * self, bool force) {
