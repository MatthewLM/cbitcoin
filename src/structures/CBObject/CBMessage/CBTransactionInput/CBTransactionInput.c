...
u_int32_t CBTransactionInputDeserialise(CBTransactionInput * self){
	...
	self->scriptObject = CBNewScriptFromReference(bytes,36 + scriptLen.size, (u_int32_t) scriptLen.val);
	self->sequence = CBByteArrayReadInt32(bytes, (u_int32_t) (36 + scriptLen.size + scriptLen.val));
	return reqLen;
}
