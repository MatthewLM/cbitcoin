...
CBIPType CBGetIPType(unsigned char * IP){
	...
	if (IP[0] == 0x20 && IP[1] == 0x01 && IP[2] == 0x0D && IP[3] == 0xB8)
		...
		return CB_IP_INVALID;
	if (! memcmp(IP, (unsigned char []){0xFE, 0x80, 0, 0, 0, 0, 0, 0}, 8))
...
bool CBIsI2P(unsigned char * IP){
	return ! memcmp(IP, (unsigned char []){0xFD, 0x60, 0xDB, 0x4D, 0xDD, 0xB5}, 6);
}
bool CBIsIPv4(unsigned char * IP){
	return ! memcmp(IP, IPv4Start, 12);
}
bool CBIsTor(unsigned char * IP){
	return ! memcmp(IP, (unsigned char []){0xFD, 0x87, 0xD8, 0x7E, 0xEB, 0x43}, 6);
}
