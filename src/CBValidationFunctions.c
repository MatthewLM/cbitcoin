...
void CBCalculateBlockWork(CBBigInt * work, int target) {
	...
	int workSeg;
	long long int temp = 0x01000000;
	
	for (int x = 0;; x++) {
		
		workSeg = (int)(temp / target);
		int i = 31 - x * 4 - zeroBytes + 4;
...
void CBCalculateMerkleRoot(unsigned char * hashes, int hashNum) {
	...
	for (int x = 0; hashNum != 1;) {
		
		if (x == hashNum - 1) {
			// Duplicate final hash
			unsigned char dup[64];
...
bool CBTransactionIsFinal(CBTransaction * tx, long long int time, long long int height) {
	...
	if (tx->lockTime) {
		...
		for (int x = 0; x < tx->inputNum; x++)
			if (tx->inputs[x]->sequence != CB_TX_INPUT_FINAL)
				return false;
...
bool CBTransactionValidateBasic(CBTransaction * tx, bool coinbase, long long int * outputValue) {
	...
	*outputValue = 0;
	...
	}else for (int x = 0; x < tx->inputNum; x++)
