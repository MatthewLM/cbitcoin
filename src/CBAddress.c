...
CBAddress * CBNewAddressFromRIPEMD160Hash(unsigned char * hash, CBBase58Prefix prefix, bool cacheString) {
...
void CBInitAddressFromRIPEMD160Hash(CBAddress * self, unsigned char * hash, CBBase58Prefix prefix, bool cacheString) {
	
	// Build address and then complete intitialisation with CBChecksumBytes
	unsigned char * data = malloc(25); // 1 Network byte, 20 hash bytes, 4 checksum bytes.
