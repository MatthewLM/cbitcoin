...
CBVersion * CBNewVersion(int32_t version, CBVersionServices services, int64_t time, CBNetworkAddress * addRecv, CBNetworkAddress * addSource, long long int nounce, CBByteArray * userAgent, int32_t blockHeight) {
...
void CBInitVersion(CBVersion * self, int32_t version, CBVersionServices services, int64_t time, CBNetworkAddress * addRecv, CBNetworkAddress * addSource, long long int nonce, CBByteArray * userAgent, int32_t blockHeight) {
...
int CBVersionDeserialise(CBVersion * self) {
	...
	int len = CBNetworkAddressDeserialise(self->addRecv, false); // No time from version message.
	...
	int x = CBByteArrayGetByte(bytes, 80); // Check length for decoding CBVarInt
	if (x > 253){
	...
	self->userAgent = CBNewByteArraySubReference(bytes, 80 + varInt.size, (int)varInt.val);
	
	// Ensure user agent uses safe characters
	CBByteArraySanitise(self->userAgent);
	
	self->blockHeight = CBByteArrayReadInt32(bytes, 80 + varInt.size + (int)varInt.val);
	return 84 + varInt.size + (int)varInt.val;
}
int CBVersionCalculateLength(CBVersion * self){
	int len = 46; // Version, services, time and receiving address.
	if (self->version >= 106) {
...
int CBVersionSerialise(CBVersion * self, bool force){
	CBByteArray * bytes = CBGetMessage(self)->bytes;
	if (! bytes) {
	...
	if (self->version >= 106) {
		...
		CBByteArraySetInt32(bytes, 80 + varInt.size + (int)varInt.val, self->blockHeight);
		// Ensure length is correct
		bytes->length = 84 + varInt.size + (int)varInt.val;
...
int CBVersionStringMaxSize(CBVersion * self){
