...
CBTransaction * CBNewTransaction(int lockTime, int version) {
...
void CBInitTransaction(CBTransaction * self, int lockTime, int version) {
...
void CBDestroyTransaction(void * vself) {
	...
	for (int x = 0; x < self->inputNum; x++)
		CBReleaseObject(self->inputs[x]);
	free(self->inputs);
	
	for (int x = 0; x < self->outputNum; x++)
...
void CBTransactionAddP2SHScript(CBTransaction * self, CBScript * p2shScript, int input) {
	
	int offset = self->inputs[input]->scriptObject->length;
...
int CBTransactionAddSignature(CBTransaction * self, CBScript * inScript, int offset, CBKeyPair * key, CBByteArray * prevOutSubScript, int input, CBSignType signType) {
	...
	int len = CBKeySign(key->privkey, hash, CBByteArrayGetData(inScript) + offset + 1);
...
void CBTransactionCalculateHash(CBTransaction * self, unsigned char * hash) {
	
	unsigned char * data = CBByteArrayGetData(CBGetMessage(self)->bytes);
...
int CBTransactionCalculateLength(CBTransaction * self) {
	
	// 8 is for version and lockTime.
	int len = 8 + CBVarIntSizeOf(self->inputNum) + CBVarIntSizeOf(self->outputNum);
...
int CBTransactionDeserialise(CBTransaction * self) {
	...
	int cursor = 4 + inputOutputLen.size;
	
	self->inputNum = (int)inputOutputLen.val;
	...
	for (int x = 0; x < self->inputNum; x++) {
		...
		int len = CBTransactionInputDeserialise(input);
	...
	self->outputNum = (int)inputOutputLen.val;
	...
	for (int x = 0; x < self->outputNum; x++) {
		...
		int len = CBTransactionOutputDeserialise(output);
		if (len == CB_DESERIALISE_ERROR){
...
unsigned char * CBTransactionGetHash(CBTransaction * self) {
...
bool CBTransactionGetInputHashForSignature(void * vself, CBByteArray * prevOutSubScript, int input, CBSignType signType, unsigned char * hash) {
	...
	int sizeOfData = 12 + prevOutSubScript->length + prevOutputSubScriptVarInt.size;
	...
	if (last5Bits == CB_SIGHASH_NONE)
		...
		int len = CBGetByteArray(self->outputs[input]->scriptObject)->length;
		...
		for (int x = 0; x < self->outputNum; x++) {
			int len = CBGetByteArray(self->outputs[x]->scriptObject)->length;
			sizeOfData += 8 + CBVarIntSizeOf(len) + len;
		}
...
void CBTransactionHashToString(CBTransaction * self, char output[CB_TX_HASH_STR_SIZE]) {
	...
	unsigned char * hash = CBTransactionGetHash(self);
	for (int x = 0; x < 20; x++)
...
bool CBTransactionInputIsStandard(CBScript * inputScript, CBScript * outputScript, CBScript * p2sh) {
	...
	int num = CBScriptIsPushOnly(inputScript);
...
bool CBTransactionIsStandard(CBTransaction * self) {
	...
	for (int x = 0; x < self->outputNum; x++) {
		...
		if (type == CB_TX_OUTPUT_TYPE_MULTISIG) {
			int pubKeyNum = CBScriptOpGetNumber(CBByteArrayGetByte(self->outputs[x]->scriptObject, self->outputs[x]->scriptObject->length - 2));
			if (pubKeyNum > 3)
...
int CBTransactionSerialise(CBTransaction * self, bool force) {
...
bool CBTransactionSignMultisigInput(CBTransaction * self, CBKeyPair * key, CBByteArray * prevOutSubScript, int input, CBSignType signType) {
...
bool CBTransactionSignPubKeyHashInput(CBTransaction * self, CBKeyPair * key, CBByteArray * prevOutSubScript, int input, CBSignType signType) {
	
	// Initilialise script data
	self->inputs[input]->scriptObject = CBNewScriptOfSize(CB_PUBKEY_SIZE + CB_MAX_DER_SIG_SIZE + 3);
	
	// Add the signature.
	int sigLen = CBTransactionAddSignature(self, self->inputs[input]->scriptObject, 0, key, prevOutSubScript, input, signType);
	if (!sigLen){
...
bool CBTransactionSignPubKeyInput(CBTransaction * self, CBKeyPair * key, CBByteArray * prevOutSubScript, int input, CBSignType signType) {
