...
CBTransactionOutput * CBNewTransactionOutput(long long int value, CBScript * script) {
...
CBTransactionOutput * CBNewTransactionOutputTakeScript(long long int value, CBScript * script) {
...
void CBInitTransactionOutput(CBTransactionOutput * self, long long int value, CBScript * script) {
...
void CBInitTransactionOutputTakeScript(CBTransactionOutput * self, long long int value, CBScript * script) {
...
int CBTransactionOutputCalculateLength(CBTransactionOutput * self) {
	
	return CBVarIntSizeOf(self->scriptObject->length) + self->scriptObject->length + 8;
	
}
int CBTransactionOutputDeserialise(CBTransactionOutput * self) {
	...
	int x = CBByteArrayGetByte(bytes, 8); // Check length for decoding CBVarInt
	if (x < 253)
		x = 9;
	else if (x == 253)
		x = 11;
	...
	int reqLen = (int)(8 + scriptLen.size + scriptLen.val);
	...
	self->scriptObject = CBNewScriptFromReference(bytes, 8 + scriptLen.size, (int) scriptLen.val);
	
	return reqLen;
	
} 
bool CBTransactionOuputGetHash(CBTransactionOutput * self, unsigned char * hash) {
...
int CBTransactionOutputSerialise(CBTransactionOutput * self) {
	...
	int reqLen = 8 + scriptLen.size + CBGetByteArray(self->scriptObject)->length;
	if (bytes->length < reqLen) {
