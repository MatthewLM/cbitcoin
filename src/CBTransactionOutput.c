...
CBTransactionOutput * CBNewTransactionOutput(uint64_t value, CBScript * script,void (*logError)(char *,...)){
	...
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewTransactionOutput\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeTransactionOutput;
	if(CBInitTransactionOutput(self,value,script,logError))
		return self;
	free(self);
	return NULL;
}
CBTransactionOutput * CBNewTransactionOutputFromData(CBByteArray * data,void (*logError)(char *,...)){
	CBTransactionOutput * self = malloc(sizeof(*self));
	if (NOT self) {
		logError("Cannot allocate %i bytes of memory in CBNewTransactionOutputFromData\n",sizeof(*self));
		return NULL;
	}
	CBGetObject(self)->free = CBFreeTransactionOutput;
	if(CBInitTransactionOutputFromData(self,data,logError))
		return self;
	free(self);
	return NULL;
}
...
uint32_t CBTransactionOutputDeserialise(CBTransactionOutput * self){
	...
	if (NOT bytes) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBTransactionOutput with no bytes.");
		return 0;
	}
	if (bytes->length < 9) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBTransactionOutput with less than 9 bytes.");
		return 0;
	}
	...
	if (x < 253)
		...
		x = 17;
	if (bytes->length < x) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBTransactionOutput with less than %i bytes required.",x);
		return 0;
	}
	...
	if (bytes->length < reqLen) {
		CBGetMessage(self)->logError("Attempting to deserialise a CBTransactionOutput with less bytes than needed according to the length for the script. %i < %i",bytes->length, reqLen);
		return 0;
	}
	...
	if (NOT self->scriptObject){
		CBGetMessage(self)->logError("Cannot create a new CBScript in CBTransactionOutputDeserialise");
		return 0;
	}
	return reqLen;
}
uint32_t CBTransactionOutputSerialise(CBTransactionOutput * self){
	...
	if (bytes->length < reqLen) {
		CBGetMessage(self)->logError("Attempting to serialise a CBTransactionOutput with less bytes than required. %i < %i\n",bytes->length, reqLen);
		return 0;
	}
