...
u_int32_t CBTransactionDeserialise(CBTransaction * self){
	...
	self->protocolVersion = CBByteArrayReadUInt32(bytes, 0);
	...
	for (u_int32_t x = 0; x < self->inputNum; x++) {
		CBTransactionInput * input = CBNewTransactionInputFromData(CBGetMessage(self)->params, CBByteArraySubReference(bytes, cursor, bytes->length-cursor), self, CBGetMessage(self)->events);
	...
	for (u_int32_t x = 0; x < self->outputNum; x++) {
		CBTransactionOutput * output = CBNewTransactionOutputFromData(CBGetMessage(self)->params, CBByteArraySubReference(bytes, cursor, bytes->length-cursor), self, CBGetMessage(self)->events);
	...
	self->lockTime = CBByteArrayReadUInt32(bytes, cursor);
	return cursor + 4;
}
u_int8_t * CBTransactionGetInputHashForSignature(CBTransaction * self, CBByteArray * prevOutSubScript, u_int32_t input, CBSignType signType){
	...
	CBByteArraySetUInt32(data, 0, self->protocolVersion);
	...
	if (signType & CB_SIGHASH_ANYONECANPAY) {
		...
		CBByteArraySetUInt32(data, 37, self->inputs[input]->outPointerIndex);
		// Add prevOutSubScript
		CBByteArrayCopyByteArray(data, 41, prevOutSubScript);
		cursor = 41 + prevOutSubScript->length;
		CBByteArraySetUInt32(data, cursor, self->inputs[input]->sequence);
		cursor += 4;
	}else{
		...
		for (u_int32_t x = 0; x < self->inputNum; x++) {
			...
			cursor += 32;
			CBByteArraySetUInt32(data, cursor, self->inputs[x]->outPointerIndex);
			cursor += 4;
			...
			if ((signType == CB_SIGHASH_NONE || signType == CB_SIGHASH_SINGLE) && x != input)
				CBByteArraySetUInt32(data, cursor, 0);
			else // SIGHASH_ALL or input index for signing sequence
				CBByteArraySetUInt32(data, cursor, self->inputs[x]->sequence);
			cursor += 4;
		}
	}
	// Copy output data
	if (last5Bits == CB_SIGHASH_NONE){
		...
		for (u_int32_t x = 0; x < input; x++) {
			CBByteArraySetUInt64(data, cursor, CB_OUTPUT_VALUE_MINUS_ONE);
			cursor += 8;
			CBVarIntEncode(data, cursor, CBVarIntFromUInt64(0));
			cursor++;
		}
		CBByteArraySetUInt64(data, cursor, self->outputs[input]->value);
		cursor += 8;
		...
		cursor += varInt.size;
		for (u_int32_t x = 0; x < self->outputNum; x++) {
			CBByteArraySetUInt64(data, cursor, self->outputs[x]->value);
			cursor += 8;
	...
	// Set lockTime
	CBByteArraySetUInt32(data, cursor, self->lockTime);
	CBByteArraySetUInt32(data, cursor + 4, signType);
...
u_int32_t CBTransactionSerialise(CBTransaction * self){
	...
	CBByteArraySetUInt32(bytes, 0, self->protocolVersion);
	...
	CBByteArraySetUInt32(bytes, cursor, self->lockTime);
	return cursor + 4;
}
...
bool CBTransactionValidateBasic(CBTransaction * self){
	...
	u_int64_t total = 0;
	for (u_int32_t x = 0; x < self->outputNum; x++) {
		if (self->outputs[x]->value > MAX_MONEY) {
			return false;
		}
