...
CBScript * CBNewScriptFromReference(CBNetworkParameters * params,CBByteArray * program,u_int32_t offset,u_int32_t len,CBEvents * events){
	...
	CBGetObject(self)->free = CBFreeScript;
	CBInitScriptFromReference(self,params,program,offset,len,events);
	return self;
}
CBScript * CBNewScriptOfSize(CBNetworkParameters * params,u_int32_t size,CBEvents * events){
	CBScript * self = malloc(sizeof(*self));
	CBGetObject(self)->free = CBFreeScript;
	CBInitScriptOfSize(self,params,size,events);
	return self;
}
CBScript * CBNewScriptFromString(CBNetworkParameters * params,char * string,CBEvents * events){
	CBScript * self = malloc(sizeof(*self));
	CBGetObject(self)->free = CBFreeScript;
	bool ok = CBInitScriptFromString(self,params,string,events);
	if (!ok) {
		free(self);
		return NULL;
	}
...
bool CBInitScriptFromReference(CBScript * self,CBNetworkParameters * params,CBByteArray * program,u_int32_t offset,u_int32_t len,CBEvents * events){
	if (!CBInitByteArraySubReference(CBGetByteArray(self), program, offset, len))
		return false;
	self->params = params;
	self->events = events;
	// Retain objects now held
	CBRetainObject(self->params);
	return true;
}
bool CBInitScriptOfSize(CBScript * self,CBNetworkParameters * params,u_int32_t size,CBEvents * events){
	if (!CBInitByteArrayOfSize(CBGetByteArray(self), size, events))
		return false;
	self->params = params;
	self->events = events;
	// Retain objects now held
...
bool CBInitScriptFromString(CBScript * self,CBNetworkParameters * params,char * string,CBEvents * events){
	...
	if (!CBInitByteArrayWithData(CBGetByteArray(self), data, dataLast, events))
		return false;
	self->params = params;
	self->events = events;
	// Retain objects now held
...
bool CBInitScriptWithData(CBScript * self,CBNetworkParameters * params,u_int8_t * data,u_int32_t size,CBEvents * events){
	if (!CBInitByteArrayWithData(CBGetByteArray(self), data, size, events))
		return false;
	self->params = params;
	self->events = events;
	// Retain objects now held
...
bool CBInitScriptWithDataCopy(CBScript * self,CBNetworkParameters * params,u_int8_t * data,u_int32_t size,CBEvents * events){
	if (!CBInitByteArrayWithDataCopy(CBGetByteArray(self), data, size, events))
		return false;
	self->params = params;
	self->events = events;
	// Retain objects now held
...
void CBFreeScript(void * vself){
	CBScript * self = vself;
	CBReleaseObject(&self->params);
...
bool CBScriptExecute(CBScript * self,CBScriptStack * stack,void * vgetHashForSig,void * transaction,u_int32_t inputIndex){
	...
	u_int8_t * (*getHashForSig)(void *, CBByteArray *, u_int32_t, CBSignType) = vgetHashForSig;
	...
	u_int32_t beginSubScript = 0;
	u_int32_t cursor = 0;
	if (CBGetByteArray(self)->length > 10000)
		return false; // Script is an illegal size.
	for (u_int8_t opCount = 0; CBGetByteArray(self)->length - cursor > 0;) {
		u_int8_t byte = CBByteArrayGetByte(CBGetByteArray(self), cursor);
			...
			if (!byte) {
				...
				if (byte == CB_SCRIPT_OP_PUSHDATA1){
					...
					if ((CBGetByteArray(self)->length - cursor) < 2)
						return false; // Not enough space.
					amount = CBByteArrayReadUInt16(CBGetByteArray(self), cursor);
					cursor += 2;
				}else{
					if ((CBGetByteArray(self)->length - cursor) < 4)
						return false; // Not enough space.
					amount = CBByteArrayReadUInt32(CBGetByteArray(self), cursor);
					cursor += 4;
				}
				// Check limitation
				if (amount > 520)
				...
				u_int32_t subScriptLen = CBGetByteArray(self)->length - beginSubScript;
				u_int8_t * subScript = malloc(subScriptLen);
				u_int8_t * sourceScript = CBByteArrayGetData(CBGetByteArray(self));
				u_int8_t * subScriptCopyPointer = subScript;
				u_int8_t * lastSeparator = sourceScript + beginSubScript;
				u_int8_t * end = sourceScript + CBGetByteArray(self)->length;
				u_int8_t * ptr = lastSeparator;
				...
				for (;;) {
					if (ptr >= end) {
						...
						u_int32_t len = ptr - lastSeparator; // Do not include code separator.
				...
				if (byte == CB_SCRIPT_OP_CHECKSIG
					...
					if (signature.data == NULL || publicKey.data == NULL) {
						...
						u_int8_t * hash = getHashForSig(transaction, subScriptByteArray, inputIndex, signType);
					...
					u_int8_t sig = 3 + numKeys; // To first signature.
					u_int8_t key = 2; // To first key.
					...
					for (u_int8_t x = 0; x < removeItemsNum; x++)
						CBScriptStackRemoveItem(stack);
				}
				if (byte == CB_SCRIPT_OP_CHECKSIG
					|| byte == CB_SCRIPT_OP_CHECKMULTISIG) {
