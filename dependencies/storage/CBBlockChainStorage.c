...
uint8_t CB_DATA_ARRAY[21];

uint64_t CBNewBlockChainStorage(char * dataDir, void (*logError)(char *,...)){
	return (uint64_t)CBNewDatabase(dataDir, logError);
}
...
bool CBBlockChainStorageDeleteBlock(void * validator, uint8_t branch, uint32_t blockIndex){
	...
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, CB_DATA_ARRAY, 20, CB_BLOCK_HASH)) {
		validatorObj->logError("Could not obtain a block hash from the block chain database.");
		return false;
	}
	// Remove data
	if (NOT CBDatabaseRemoveValue(database, CB_BLOCK_KEY)){
		validatorObj->logError("Could not remove block value from database.");
		return false;
	}
	...
	if (NOT CBDatabaseRemoveValue(database, CB_BLOCK_HASH_INDEX_KEY)){
		validatorObj->logError("Could not remove block hash index reference from database.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageDeleteUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex){
	...
	if (NOT CBDatabaseRemoveValue(database, CB_UNSPENT_OUTPUT_KEY)) {
		validatorObj->logError("Could not remove an unspent output reference from storage.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageDeleteTransactionRef(void * validator, uint8_t * txHash){
	...
	if (NOT CBDatabaseRemoveValue(database, CB_TRANSACTION_INDEX_KEY)) {
		validatorObj->logError("Could not remove a transaction reference from storage.");
		return false;
	}
	return true;
}
...
bool CBBlockChainStorageGetBlockLocation(void * validator, uint8_t * blockHash, uint8_t * branch, uint32_t * index){
	...
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_HASH_INDEX_KEY, CB_DATA_ARRAY, 5, 0)) {
		validatorObj->logError("Could not read a block hash reference from the block chain database.");
		return false;
	}
...
uint32_t CBBlockChainStorageGetBlockTime(void * validator, uint8_t branch, uint32_t blockIndex){
	...
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, CB_DATA_ARRAY, 4, CB_BLOCK_TIME)) {
		validatorObj->logError("Could not read the time for a block.");
		return 0;
	}
...
uint32_t CBBlockChainStorageGetBlockTarget(void * validator, uint8_t branch, uint32_t blockIndex){
	...
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, CB_DATA_ARRAY, 4, CB_BLOCK_TARGET)) {
		validatorObj->logError("Could not read the target for a block.");
		return 0;
	}
	return CBArrayToInt32(CB_DATA_ARRAY, 0);
}
bool CBBlockChainStorageLoadBasicValidator(void * validator){
	...
	if (NOT CBDatabaseReadValue(database, CB_VALIDATOR_INFO_KEY, CB_DATA_ARRAY, 4, 0)){
		validatorObj->logError("There was an error when reading the validator information from storage.");
		return false;
	}
...
void * CBBlockChainStorageLoadBlock(void * validator, uint32_t blockID, uint32_t branch){
	...
	// Get block data
	CBByteArray * data = CBNewByteArrayOfSize(blockDataLen, validatorObj->logError);
	if (NOT data) {
		validatorObj->logError("Could not initialise a byte array for loading a block.");
		return NULL;
	}
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, CBByteArrayGetData(data), blockDataLen, CB_BLOCK_START)){
		validatorObj->logError("Could not read a block from the database.");
		CBReleaseObject(data);
		return NULL;
	}
	// Make and return the block
	CBBlock * block = CBNewBlockFromData(data, validatorObj->logError);
	CBReleaseObject(data);
	if (NOT block) {
		validatorObj->logError("Could not create a block object when loading a block.");
		return NULL;
	}
	return block;
}
bool CBBlockChainStorageLoadBranch(void * validator, uint8_t branchNum){
	...
	if (NOT CBDatabaseReadValue(database, CB_BRANCH_KEY, CB_DATA_ARRAY, 21, 0)) {
		validatorObj->logError("There was an error when reading the data for a branch's information.");
		return false;
	}
...
bool CBBlockChainStorageLoadBranchWork(void * validator, uint8_t branchNum){
	...
	if (NOT CBBigIntAlloc(&validatorObj->branches[branchNum].work, workLen)){
		validatorObj->logError("There was an error when allocating memory for a branch's work.");
		return false;
	}
	...
	if (NOT CBDatabaseReadValue(database, CB_WORK_KEY, validatorObj->branches[branchNum].work.data, workLen, 0)) {
		validatorObj->logError("There was an error when reading the work for a branch.");
...
bool CBBlockChainStorageLoadOrphan(void * validator, uint8_t orphanNum){
	...
	CBByteArray * orphanData = CBNewByteArrayOfSize(len, validatorObj->logError);
	if (NOT orphanData) {
		validatorObj->logError("There was an error when initialising a byte array for an orphan.");
		return false;
	}
	if (NOT CBDatabaseReadValue(database, CB_ORPHAN_KEY, CBByteArrayGetData(orphanData), len, 0)) {
		validatorObj->logError("There was an error when reading the data for an orphan.");
		CBReleaseObject(orphanData);
		return false;
	}
	validatorObj->orphans[orphanNum] = CBNewBlockFromData(orphanData, validatorObj->logError);
	if (NOT validatorObj->orphans[orphanNum]) {
		validatorObj->logError("There was an error when creating a block object for an orphan.");
		CBReleaseObject(orphanData);
		return false;
	}
	CBReleaseObject(orphanData);
...
bool CBBlockChainStorageLoadOutputs(void * validator, uint8_t * txHash, uint8_t * data, uint32_t * dataAllocSize, uint32_t * position){
	...
	if (NOT CBDatabaseReadValue(database, CB_TRANSACTION_INDEX_KEY, CB_DATA_ARRAY, 14, 0)) {
		validatorObj->logError("Could not read a transaction reference from the transaction index.");
		return false;
	}
	...
	*position = CBArrayToInt32(CB_DATA_ARRAY, CB_TRANSACTION_REF_POSITION_OUPTUTS);
	...
	if (CBArrayToInt32(CB_DATA_ARRAY, CB_TRANSACTION_REF_LENGTH_OUTPUTS) > *dataAllocSize) {
		*dataAllocSize = CBArrayToInt32(CB_DATA_ARRAY, CB_TRANSACTION_REF_LENGTH_OUTPUTS);
		...
		if (NOT data) {
			validatorObj->logError("Could not allocate memory for reading a transaction.");
			return false;
		}
	}
	...
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, data, CBArrayToInt32(CB_DATA_ARRAY, CB_TRANSACTION_REF_LENGTH_OUTPUTS), CB_BLOCK_START + *position)) {
		validatorObj->logError("Could not read a transaction from the block-chain database.");
		return false;
	}
	return true;
}
void * CBBlockChainStorageLoadUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex, bool * coinbase, uint32_t * outputHeight){
	...
	if (NOT CBDatabaseReadValue(database, CB_UNSPENT_OUTPUT_KEY, CB_DATA_ARRAY, 8, 0)) {
		validatorObj->logError("Cannot read unspent output information from the block chain database");
		return NULL;
	}
	uint32_t outputPosition = CB_DATA_ARRAY[CB_UNSPENT_OUTPUT_REF_POSITION];
	uint32_t outputLength = CB_DATA_ARRAY[CB_UNSPENT_OUTPUT_REF_LENGTH];
	// Now read data for the transaction
	memcpy(CB_TRANSACTION_INDEX_KEY + 2, txHash, 32);
	if (NOT CBDatabaseReadValue(database, CB_TRANSACTION_INDEX_KEY, CB_DATA_ARRAY, 14, 0)) {
		validatorObj->logError("Cannot read a transaction reference from the transaction index.");
		return NULL;
	}
	...
	*outputHeight = validatorObj->branches[outputBranch].startHeight + outputBlockIndex;
	...
	// Get output data
	CBByteArray * outputBytes = CBNewByteArrayOfSize(outputLength, validatorObj->logError);
	if (NOT outputBytes) {
		validatorObj->logError("Could not create  CBByteArray for an unspent output.");
		return NULL;
	}
	if (NOT CBDatabaseReadValue(database, CB_BLOCK_KEY, CBByteArrayGetData(outputBytes), outputLength, CB_BLOCK_START + outputPosition)) {
		validatorObj->logError("Could not read an unspent output");
		CBReleaseObject(outputBytes);
		return NULL;
	}
	// Create output object
	CBTransactionOutput * output = CBNewTransactionOutputFromData(outputBytes, validatorObj->logError);
	CBReleaseObject(outputBytes);
	if (NOT output) {
		validatorObj->logError("Could not create an object for an unspent output");
		return NULL;
	}
	if (NOT CBTransactionOutputDeserialise(output)) {
		validatorObj->logError("Could not deserialise an unspent output");
		return NULL;
	}
	return output;
}
bool CBBlockChainStorageMoveBlock(void * validator, uint8_t branch, uint32_t blockIndex, uint8_t newBranch, uint32_t newIndex){
	...
	if (NOT CBDatabaseChangeKey(database, CB_BLOCK_KEY, CB_NEW_BLOCK_KEY)) {
		validatorObj->logError("Could not move a block location in the block-chain database.");
		return false;
	}
	return true;
}
...
bool CBBlockChainStorageSaveBasicValidator(void * validator){
	CBFullValidator * validatorObj = validator;
	CBDatabase * database = (CBDatabase *)validatorObj->storage;
	CB_DATA_ARRAY[CB_VALIDATION_FIRST_ORPHAN] = 0;
	CB_DATA_ARRAY[CB_VALIDATION_NUM_ORPHANS] = 0;
	CB_DATA_ARRAY[CB_VALIDATION_MAIN_BRANCH] = 0;
	CB_DATA_ARRAY[CB_VALIDATION_NUM_BRANCHES] = 1;
	if (NOT CBDatabaseWriteValue(database, CB_VALIDATOR_INFO_KEY, CB_DATA_ARRAY, 4)) {
		validatorObj->logError("Could not write the initial basic validation data.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveBlock(void * validator, void * block, uint8_t branch, uint32_t blockIndex){
	...
	if (NOT CBDatabaseWriteConcatenatedValue((CBDatabase *)validatorObj->storage, CB_BLOCK_KEY, 2, dataParts, dataSizes)) {
		validatorObj->logError("Could not write a block to the block-chain database.");
		return false;
	}
	...
	if (NOT CBDatabaseWriteValue((CBDatabase *)validatorObj->storage, CB_BLOCK_HASH_INDEX_KEY, CB_DATA_ARRAY, 5)) {
		validatorObj->logError("Could not write a block hash to the block-chain database block hash index.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveBranch(void * validator, uint8_t branch){
	...
	if (NOT CBDatabaseWriteValue(database, CB_BRANCH_KEY, CB_DATA_ARRAY, 21)) {
		validatorObj->logError("Could not write branch information.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveBranchWork(void * validator, uint8_t branch){
	...
	if (NOT CBDatabaseWriteValue(database, CB_WORK_KEY, validatorObj->branches[branch].work.data, validatorObj->branches[branch].work.length)) {
		validatorObj->logError("Could not write branch work.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveOrphan(void * validator, void * block, uint8_t orphanNum){
	...
	if (NOT CBDatabaseWriteValue(database, CB_ORPHAN_KEY, CBByteArrayGetData(CBGetMessage(blockObj)->bytes), CBGetMessage(blockObj)->bytes->length)) {
		validatorObj->logError("Could not write an orphan.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveTransactionRef(void * validator, uint8_t * txHash, uint8_t branch, uint32_t blockIndex, uint32_t outputPos, uint32_t outputsLen, bool coinbase){
	...
	if (NOT CBDatabaseWriteValue(database, CB_TRANSACTION_INDEX_KEY, CB_DATA_ARRAY, 14)) {
		validatorObj->logError("Could not write transaction reference to transaction index.");
		return false;
	}
	return true;
}
bool CBBlockChainStorageSaveUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex, uint32_t position, uint32_t length){
	...
	if (NOT CBDatabaseWriteValue(database, CB_UNSPENT_OUTPUT_KEY, CB_DATA_ARRAY, 8)) {
		validatorObj->logError("Could not write new unspent output reference into the database.");
		return false;
	}
	return true;
}
