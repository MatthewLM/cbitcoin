...
typedef struct{
	...
	unsigned char hash[32]; /**< The hash for this block. NULL if it needs to be calculated or set. */
	...
	unsigned int time; /**< Timestamp for the block. The network uses 32 bits. ??? Add overflow detection. If previous block timestamps are above some threshold, assume an overflow. But this will possibly be fixed in the future in the protocol, so will have to be changed anyway. */
	int target; /**< The compact target representation. */
	unsigned int nonce; /**< Nounce used in generating the block. */
	int transactionNum; /**< Number of transactions in the block. */
