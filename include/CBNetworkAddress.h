...
typedef struct{
	...
	unsigned long long int lastSeen; /**< The timestamp the address was last seen */
	int penalty; /**< Addresses are selected partially by their time last seen, but the time last seen is adjusted with a penalty for bad behaviour. This is taken awat from the time last seen. */
	...
	int bucket; /**< The bucket number for this address. */
