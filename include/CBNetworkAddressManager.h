...
typedef struct{
	...
	int addrNum; /**< Number of addresses. */
	CBAssociativeArray peers; /**< The peers sorted by CBNetworkAddressIPPortCompare */
	int peersNum; /**< Number of connected peers. */
	CBAssociativeArray peerTimeOffsets; /**< The time offsets of the peers */
	int timeOffsetNum; /**< The number of time offsets recorded. */
	int16_t networkTimeOffset; /**< Offset to get from system time to network time. */
	int maxAddressesInBucket; /**< Maximum number of addresses that can be stored in a single bucket. */
	long long int secret; /**< Securely generated pseudo-random number to generate a secret used to mix-up groups into different buckets. */
