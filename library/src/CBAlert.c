...
uint32_t CBAlertDeserialise(CBAlert * self) {
	...
	uint8_t size = CBByteArrayReadVarIntSize(bytes, cursor);
	
	if (payloadLen.val < cursor + size + 2 - payloadLen.size) {
	...
	if (payloadLen.val >= cursor + size + 1 - payloadLen.size) {
				...
				if (payloadLen.val == cursor + reservedLen.val - payloadLen.size) {
					...
					if (bytes->length >= cursor + size) {
						...
						if (bytes->length >= cursor + sigLen.val){
							...
							self->payload = CBNewByteArraySubReference(bytes, payloadLen.size, (uint32_t)payloadLen.val);
