...
bool CBBlockChainStorageChangeUnspentOutputsNum(CBBlockChainStorage * storage, uint8_t * txHash, bool add) {
	...
	CBInt32ToArray(data, 0, CBArrayToInt32(data, 0) + (add ? 1 : -1));
...
bool CBBlockChainStorageDeleteUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex, bool decrement) {
	...
	// Place transaction hash and output index into the key
	...
	if (decrement
		...
		&& ! CBBlockChainStorageChangeUnspentOutputsNum(storageObj, txHash, false)) {
...
bool CBBlockChainStorageDeleteTransactionRef(void * validator, uint8_t * txHash) {
	...
	// Else this was the last instance so remove it from storage
...
void * CBBlockChainStorageGetBlockHeader(void * validator, uint8_t branch, uint32_t blockIndex) {
	...
	uint8_t varIntSize = CBByteArrayReadVarInt(data, 80).size;
...
uint32_t CBBlockChainStorageGetBlockTime(void * validator, uint8_t branch, uint32_t blockIndex) {
	...
	return CBBlockChainStorageGetUInt32(((CBValidator *)validator)->storage.ptr, branch, blockIndex, CB_BLOCK_TIME);
	
}

uint32_t CBBlockChainStorageGetBlockTarget(void * validator, uint8_t branch, uint32_t blockIndex) {
	
	return CBBlockChainStorageGetUInt32(((CBValidator *)validator)->storage.ptr, branch, blockIndex, CB_BLOCK_TARGET);
...
uint32_t CBBlockChainStorageGetUInt32(CBBlockChainStorage * storage, uint8_t branch, uint32_t blockIndex, CBBlockOffsets offset) {
	...
	if (CBDatabaseReadValue(storage->blockIndex, key, data, 4, offset, false) != CB_DATABASE_INDEX_FOUND) {
...
void * CBBlockChainStorageLoadUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex, bool * coinbase, uint32_t * outputHeight) {
	...
	*outputHeight = validatorObj->branches[outputBranch].startHeight + outputBlockIndex;
	...
	CBBlock * theBlock = CBBlockChainStorageLoadBlock(validator, outputBlockIndex, outputBranch);
	...
	if (CBTransactionOutputDeserialise(output) == CB_DESERIALISE_ERROR) {
		CBLogError("Could not deserialise an unspent output %p", theBlock);
...
bool CBBlockChainStorageSaveBlockHeader(void * validator, void * block, uint8_t branch, uint32_t blockIndex) {
	...
	uint32_t dataSizes[3] = {32, 80 + CBByteArrayReadVarInt(CBGetMessage(blockObj)->bytes, 80).size, 1};
...
bool CBBlockChainStorageSaveTransactionRef(void * validator, uint8_t * txHash, uint8_t branch, uint32_t blockIndex, uint32_t outputPos, uint32_t outputsLen, bool coinbase, uint32_t numOutputs) {
	...
	if (len != CB_DOESNT_EXIST) {
		...
		if (CBDatabaseReadValue(storageObj->txIndex, txHash, data + 4, 4, CB_TX_REF_INSTANCE_COUNT, false) != CB_DATABASE_INDEX_FOUND) {
			CBLogError("Could not read a transaction reference instance count from the transaction index.");
			return false;
		}
...
bool CBBlockChainStorageSaveUnspentOutput(void * validator, uint8_t * txHash, uint32_t outputIndex, uint32_t position, uint32_t length, bool increment) {
	...
	if (increment
		...
		&& ! CBBlockChainStorageChangeUnspentOutputsNum(storageObj, txHash, true)) {
