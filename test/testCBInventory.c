...
int main(){
	...
	if (inv->items[0][0]->type != CB_INVENTORY_ITEM_TX) {
		printf("DESERIALISATION FIRST TYPE FAIL\n");
		return 1;
	}
	if (memcmp(CBByteArrayGetData(inv->items[0][0]->hash), (uint8_t []){0xFB, 0x30, 0xB1, 0x9B, 0x3A, 0x0F, 0x82, 0x4A, 0xF1, 0x2B, 0x6E, 0xA4, 0x72, 0xBA, 0x9B, 0x3A, 0x67, 0xBA, 0xF2, 0xD8, 0x2E, 0x18, 0x06, 0x9D, 0x4A, 0x1B, 0x54, 0xA3, 0xD8, 0x9C, 0x43, 0xCF}, 32)) {
		printf("DESERIALISATION FIRST HASH FAIL\n0x");
		uint8_t * d = CBByteArrayGetData(inv->items[0][0]->hash);
		for (int x = 0; x < 32; x++) {
			printf("%.2X", d[x]);
		}
		printf("\n!=\n0x");
	...
	if (inv->items[0][1]->type != CB_INVENTORY_ITEM_BLOCK) {
		printf("DESERIALISATION SECOND TYPE FAIL\n");
		return 1;
	}
	if (memcmp(CBByteArrayGetData(inv->items[0][1]->hash), (uint8_t []){0xFA, 0x30, 0xB1, 0x9B, 0x35, 0x0F, 0x82, 0x4A, 0x92, 0x2B, 0x6E, 0xA4, 0x72, 0xBA, 0x9C, 0x3A, 0x67, 0xBA, 0xF2, 0x18, 0x12, 0x18, 0xF2, 0x9D, 0x4A, 0x1C, 0x54, 0xA3, 0xD8, 0x9C, 0x43, 0xBA}, 32)) {
		printf("DESERIALISATION SECOND HASH FAIL\n0x");
		uint8_t * d = CBByteArrayGetData(inv->items[0][1]->hash);
		for (int x = 0; x < 32; x++) {
			printf("%.2X", d[x]);
		}
		printf("\n!=\n0x");
	...
	if (inv->items[0][2]->type != CB_INVENTORY_ITEM_ERROR) {
		printf("DESERIALISATION THIRD TYPE FAIL\n");
		return 1;
	}
	if (memcmp(CBByteArrayGetData(inv->items[0][2]->hash), (uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 32)) {
		printf("DESERIALISATION THIRD HASH FAIL\n0x");
		uint8_t * d = CBByteArrayGetData(inv->items[0][2]->hash);
		for (int x = 0; x < 32; x++) {
			printf("%.2X", d[x]);
		}
		printf("\n!=\n0x");
	...
	CBReleaseObject(inv->items[0][0]->hash);
	inv->items[0][0]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0xFB, 0x30, 0xB1, 0x9B, 0x3A, 0x0F, 0x82, 0x4A, 0xF1, 0x2B, 0x6E, 0xA4, 0x72, 0xBA, 0x9B, 0x3A, 0x67, 0xBA, 0xF2, 0xD8, 0x2E, 0x18, 0x06, 0x9D, 0x4A, 0x1B, 0x54, 0xA3, 0xD8, 0x9C, 0x43, 0xCF}, 32);
	CBReleaseObject(inv->items[0][1]->hash);
	inv->items[0][1]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0xFA, 0x30, 0xB1, 0x9B, 0x35, 0x0F, 0x82, 0x4A, 0x92, 0x2B, 0x6E, 0xA4, 0x72, 0xBA, 0x9C, 0x3A, 0x67, 0xBA, 0xF2, 0x18, 0x12, 0x18, 0xF2, 0x9D, 0x4A, 0x1C, 0x54, 0xA3, 0xD8, 0x9C, 0x43, 0xBA}, 32);
	CBReleaseObject(inv->items[0][2]->hash);
	inv->items[0][2]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 32);
