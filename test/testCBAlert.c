...
int main(){
	...
	CBByteArray * bytes = CBNewByteArrayWithDataCopy(data, 188, &events);
	CBAlert * alert = CBNewAlertFromData(bytes, &events);
	...
	alert->displayedComment = CBNewByteArrayWithDataCopy((u_int8_t *)"See bitcoin.org/feb20 if you have trouble connecting after 20 February", 70, &events);
	CBReleaseObject(&alert->signature);
	alert->signature = CBNewByteArrayWithDataCopy((u_int8_t []){0x30,0x45,0x02,0x21,0x00,0x83,0x89,0xdf,0x45,0xF0,0x70,0x3F,0x39,0xEC,0x8C,0x1C,0xC4,0x2C,0x13,0x81,0x0F,0xFC,0xAE,0x14,0x99,0x5B,0xB6,0x48,0x34,0x02,0x19,0xE3,0x53,0xB6,0x3B,0x53,0xEB,0x02,0x20,0x09,0xEC,0x65,0xE1,0xC1,0xAA,0xEE,0xC1,0xFD,0x33,0x4C,0x6B,0x68,0x4B,0xDE,0x2B,0x3F,0x57,0x30,0x60,0xD5,0xb7,0x0C,0x3A,0x46,0x72,0x33,0x26,0xE4,0xE8,0xA4,0xF1}, 71, &events);
