...
int main(){
	...
	blockHeaders->blockHeaders[0]->prevBlockHash = CBNewByteArrayWithDataCopy((uint8_t []){0xFA,0x30,0xB1,0x9B,0x35,0x0F,0x82,0x4A,0x92,0x2B,0x6E,0xA4,0x72,0xBA,0x9C,0x3A,0x67,0xBA,0xF2,0x18,0x12,0x18,0xF2,0x9D,0x4A,0x1C,0x54,0xA3,0xD8,0x9C,0x43,0xBA}, 32, &events);
	CBReleaseObject(blockHeaders->blockHeaders[0]->merkleRoot);
	blockHeaders->blockHeaders[0]->merkleRoot = CBNewByteArrayWithDataCopy((uint8_t []){0xC8,0x57,0xA1,0xFB,0xCA,0x8F,0x22,0x4B,0x21,0xB5,0xFE,0x94,0x22,0xB0,0x7C,0xBA,0x87,0x2A,0xB2,0xF8,0xCE,0x39,0x56,0xBD,0x99,0x4A,0x44,0x23,0x2F,0x8C,0xA3,0xFF}, 32, &events);
	CBReleaseObject(blockHeaders->blockHeaders[1]->prevBlockHash);
	blockHeaders->blockHeaders[1]->prevBlockHash = CBNewByteArrayWithDataCopy((uint8_t []){0xFA,0x30,0xB1,0x9B,0x35,0x0F,0x82,0x4A,0x92,0x2B,0x6E,0xA4,0x72,0xBA,0x9C,0x3A,0x67,0xBA,0xF2,0x18,0x12,0x18,0xF2,0x9D,0x4A,0x1C,0x54,0xA3,0xD8,0x9C,0x43,0xBA}, 32, &events);
	CBReleaseObject(blockHeaders->blockHeaders[1]->merkleRoot);
	blockHeaders->blockHeaders[1]->merkleRoot = CBNewByteArrayWithDataCopy((uint8_t []){0xE8,0x47,0xA9,0xBB,0xD8,0x4F,0x72,0x9B,0x11,0xB5,0xFC,0x96,0x42,0xE0,0x9C,0xBA,0xA7,0xF9,0xE2,0xFD,0xC8,0x49,0xB6,0xDD,0x49,0x8A,0xE4,0xA3,0x3F,0x0C,0x23,0x1F}, 32, &events);
