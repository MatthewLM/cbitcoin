...
int main(){
	...
	remove("./2.dat");
	remove("./log.dat");
	CBDatabase * storage = CBNewDatabase("./");
	if (NOT storage) {
	...
	uint64_t file = CBFileOpen("./0.dat", false);
	uint8_t data[105];
	...
	storage = (CBDatabase *)CBNewDatabase("./");
	if (storage->numValues) {
