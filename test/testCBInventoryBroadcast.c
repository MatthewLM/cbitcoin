...
int main(){
	...
	CBByteArray * bytes = CBNewByteArrayWithDataCopy(data, 109, logError);
	CBInventoryBroadcast * invBroadcast = CBNewInventoryBroadcastFromData(bytes, logError);
	...
	invBroadcast->items[0]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0xFB,0x30,0xB1,0x9B,0x3A,0x0F,0x82,0x4A,0xF1,0x2B,0x6E,0xA4,0x72,0xBA,0x9B,0x3A,0x67,0xBA,0xF2,0xD8,0x2E,0x18,0x06,0x9D,0x4A,0x1B,0x54,0xA3,0xD8,0x9C,0x43,0xCF}, 32, logError);
	CBReleaseObject(invBroadcast->items[1]->hash);
	invBroadcast->items[1]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0xFA,0x30,0xB1,0x9B,0x35,0x0F,0x82,0x4A,0x92,0x2B,0x6E,0xA4,0x72,0xBA,0x9C,0x3A,0x67,0xBA,0xF2,0x18,0x12,0x18,0xF2,0x9D,0x4A,0x1C,0x54,0xA3,0xD8,0x9C,0x43,0xBA}, 32, logError);
	CBReleaseObject(invBroadcast->items[2]->hash);
	invBroadcast->items[2]->hash = CBNewByteArrayWithDataCopy((uint8_t []){0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}, 32, logError);
