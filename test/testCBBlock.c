...
int main(){
	...
	// Test genesis block
	CBByteArray * genesisMerkleRoot = CBNewByteArrayWithDataCopy((uint8_t []){0x3B,0xA3,0xED,0xFD,0x7A,0x7B,0x12,0xB2,0x7A,0xC7,0x2C,0x3E,0x67,0x76,0x8F,0x61,0x7F,0xC8,0x1B,0xC3,0x88,0x8A,0x51,0x32,0x3A,0x9F,0xB8,0xAA,0x4B,0x1E,0x5E,0x4A}, 32, &events);
	...
	if(NOT CBByteArrayEquals(genesisBlock->hash, calcHash)){
		printf("GENESIS BLOCK HASH FAIL\n0x");
		uint8_t * d = CBByteArrayGetData(genesisBlock->hash);
		for (int x = 0; x < 32; x++) {
			printf("%.2X",d[x]);
		}
		printf("\n!=\n0x");
	...
	if (NOT CBByteArrayEquals(genesisBlock->merkleRoot, genesisMerkleRoot)) {
	...
	if (NOT CBByteArrayEquals(genesisCoinBase->outputs[0]->scriptObject, genesisOutScript)) {
