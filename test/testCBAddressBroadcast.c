...
int main(){
	...
	if (memcmp(CBByteArrayGetData(add->addresses[0]->sockAddr.ip), (unsigned char []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x0A, 0x00, 0x00, 0x01}, 16)) {
		printf("DESERIALISATION FIRST IP FAIL\n0x");
		unsigned char * d = CBByteArrayGetData(add->addresses[0]->sockAddr.ip);
		for (int x = 0; x < 16; x++) {
			printf("%.2X", d[x]);
		}
		printf("\n!=\n0x");
	...
	if (memcmp(CBByteArrayGetData(add->addresses[1]->sockAddr.ip), (unsigned char []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x24, 0x60, 0xA2, 0x08}, 16)) {
		printf("DESERIALISATION SECOND IP FAIL\n0x");
		unsigned char * d = CBByteArrayGetData(add->addresses[1]->sockAddr.ip);
		for (int x = 0; x < 16; x++) {
			printf("%.2X", d[x]);
		}
		printf("\n!=\n0x");
		d = (unsigned char []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x24, 0x60, 0xA2, 0x08};
		for (int x = 0; x < 16; x++) {
			printf("%.2X", d[x]);
		}
		return 1;
	...
	add->addresses[0]->sockAddr.ip = CBNewByteArrayWithDataCopy((unsigned char []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x0A, 0x00, 0x00, 0x01}, 16);
	CBReleaseObject(add->addresses[1]->sockAddr.ip);
	add->addresses[1]->sockAddr.ip = CBNewByteArrayWithDataCopy((unsigned char []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x24, 0x60, 0xA2, 0x08}, 16);
