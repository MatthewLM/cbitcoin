...
int main(){
	...
	if(memcmp(hash, CBByteArrayGetData(input->prevOut.hash), 32)){
		...
		printf(" != 0x");
		unsigned char * d = CBByteArrayGetData(input->prevOut.hash);
		for (int x = 0; x < 32; x++) {
			printf("%.2x", d[x]);
		}
		return 1;
	...
	// Test serialisation
	CBScript * scriptObj = CBNewScriptWithDataCopy((unsigned char []){CB_SCRIPT_OP_TRUE}, 1 );
	...
	if (CBByteArrayCompare(bytes, CBGetMessage(input)->bytes)) {
		...
		unsigned char * d = CBByteArrayGetData(CBGetMessage(input)->bytes);
		for (int x = 0; x < 42; x++) {
			printf("%.2x", d[x]);
		}
		printf("\n!=\n0x");
	...
	unsigned long long int value = rand();
	...
	if (output->value != value) {
		printf("CBTransactionOutput DESERIALISED value INCORRECT: %lli != %lli\n", output->value, value);
		return 1;
	}
	...
	if (CBByteArrayCompare(bytes, CBGetMessage(output)->bytes)) {
		...
		unsigned char * d = CBByteArrayGetData(CBGetMessage(output)->bytes);
		for (int x = 0; x < 14; x++) {
			printf("%.2x", d[x]);
		}
		printf("\n!=\n0x");
	...
	// Test deserialisation
	// Set script data
	unsigned char * scripts[6];
	scripts[0] = (unsigned char []){CB_SCRIPT_OP_4, CB_SCRIPT_OP_2, CB_SCRIPT_OP_SUB, CB_SCRIPT_OP_3, CB_SCRIPT_OP_MAX, CB_SCRIPT_OP_3, CB_SCRIPT_OP_EQUAL};
	scripts[1] = (unsigned char []){CB_SCRIPT_OP_1, CB_SCRIPT_OP_3, CB_SCRIPT_OP_7, CB_SCRIPT_OP_ROT, CB_SCRIPT_OP_SUB, CB_SCRIPT_OP_ADD, CB_SCRIPT_OP_9, CB_SCRIPT_OP_EQUAL};
	scripts[2] = (unsigned char []){CB_SCRIPT_OP_TRUE, CB_SCRIPT_OP_IF, CB_SCRIPT_OP_3, CB_SCRIPT_OP_ELSE, CB_SCRIPT_OP_2, CB_SCRIPT_OP_ENDIF, CB_SCRIPT_OP_3, CB_SCRIPT_OP_EQUAL};
	scripts[3] = (unsigned char []){CB_SCRIPT_OP_3, CB_SCRIPT_OP_DUP, CB_SCRIPT_OP_DUP, CB_SCRIPT_OP_ADD, CB_SCRIPT_OP_ADD, CB_SCRIPT_OP_9, CB_SCRIPT_OP_EQUAL};
	scripts[4] = (unsigned char []){CB_SCRIPT_OP_3, CB_SCRIPT_OP_3, CB_SCRIPT_OP_SUB, CB_SCRIPT_OP_NOT, CB_SCRIPT_OP_TRUE, CB_SCRIPT_OP_BOOLAND};
	for (int y = 0; y < 32; y++) {
		hash[y] = rand();
	}
	int randInt = rand();
	long long int randInt64 = rand();
	...
	for (int x = 0; x < 3; x++) {
		if(memcmp(hash, CBByteArrayGetData(tx->inputs[x]->prevOut.hash), 32)){
			...
			printf(" != 0x");
			unsigned char * d = CBByteArrayGetData(tx->inputs[x]->prevOut.hash);
			for (int x = 0; x < 32; x++) {
				printf("%.2x", d[x]);
			}
			return 1;
		}
		if (tx->inputs[x]->prevOut.index != (unsigned int)randInt) {
		...
		if (tx->inputs[x]->sequence != (unsigned int)randInt) {
			printf("TX CBTransactionInput %i DESERIALISED sequence INCORRECT: %i != %i\n", x, tx->inputs[x]->sequence, randInt);
			return 1;
		}
	}
	for (int x = 0; x < 2; x++) {
		if (tx->outputs[x]->value != (unsigned int)randInt64) {
			printf("TX CBTransactionOutput %i DESERIALISED value INCORRECT: %lli != %lli\n", x, tx->outputs[x]->value, randInt64);
			return 1;
		}
		stack = CBNewEmptyScriptStack();
	...
	if (CBByteArrayCompare(bytes, CBGetMessage(tx)->bytes)) {
		...
		unsigned char * d = CBByteArrayGetData(CBGetMessage(tx)->bytes);
		for (int x = 0; x < 187; x++) {
			printf("%.2x", d[x]);
		}
		printf("\n!=\n0x");
	...
	for (int x = 0; x < 2; x++) if (CBGetMessage(tx->outputs[x])->bytes->sharedData != CBGetMessage(tx)->bytes->sharedData){
	...
	EC_KEY * keys[21];
	unsigned char * pubKeys[21];
	unsigned char pubSizes[21];
	...
	for (int x = 0; x < 21; x++){
		...
		unsigned char * pubKey2 = pubKeys[x];
	...
	// Make outputs
	for (int x = 0; x < 4; x++){
		CBScript * outputsScript = CBNewScriptWithDataCopy((unsigned char []){CB_SCRIPT_OP_TRUE}, 1);
		CBTransactionTakeOutput(tx, CBNewTransactionOutput(1000, outputsScript));
		CBReleaseObject(outputsScript);
	}
	// Make inputs
	CBSha256((unsigned char []){0x56, 0x21, 0xF2}, 3, hash);
	for (int x = 0; x < 4; x++){
	...
	for (int x = 0; x < 4; x++) {
		...
		unsigned char * signature = malloc(sigSizes[x] + 1);
	...
	scriptObj = CBNewScriptWithDataCopy((unsigned char []){CB_SCRIPT_OP_TRUE}, 1);
	CBTransactionTakeOutput(tx, CBNewTransactionOutput(80, scriptObj));
	CBReleaseObject(scriptObj);
	// Make previous output script with OP_CHECKMULTISIG
	CBScript * outputScript = CBNewScriptWithDataCopy((unsigned char []){CB_SCRIPT_OP_CHECKMULTISIG}, 1);
	// Make signatures
	unsigned char * signatures[21];
	...
	bytes = CBNewByteArrayWithDataCopy((unsigned char []){0x1, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xff, 0xff, 0xff, 0xff, 0x7, 0x4, 0xff, 0xff, 0x0, 0x1d, 0x1, 0x4, 0xff, 0xff, 0xff, 0xff, 0x1, 0x0, 0xf2, 0x5, 0x2a, 0x1, 0x0, 0x0, 0x0, 0x43, 0x41, 0x4, 0x96, 0xb5, 0x38, 0xe8, 0x53, 0x51, 0x9c, 0x72, 0x6a, 0x2c, 0x91, 0xe6, 0x1e, 0xc1, 0x16, 0x0, 0xae, 0x13, 0x90, 0x81, 0x3a, 0x62, 0x7c, 0x66, 0xfb, 0x8b, 0xe7, 0x94, 0x7b, 0xe6, 0x3c, 0x52, 0xda, 0x75, 0x89, 0x37, 0x95, 0x15, 0xd4, 0xe0, 0xa6, 0x4, 0xf8, 0x14, 0x17, 0x81, 0xe6, 0x22, 0x94, 0x72, 0x11, 0x66, 0xbf, 0x62, 0x1e, 0x73, 0xa8, 0x2c, 0xbf, 0x23, 0x42, 0xc8, 0x58, 0xee, 0xac, 0x0, 0x0, 0x0, 0x0}, 134);
	tx = CBNewTransactionFromData(bytes);
	if (memcmp(CBTransactionGetHash(tx), (unsigned char []){0x98, 0x20, 0x51, 0xfD, 0x1E, 0x4B, 0xA7, 0x44, 0xBB, 0xBE, 0x68, 0x0E, 0x1F, 0xEE, 0x14, 0x67, 0x7B, 0xA1, 0xA3, 0xC3, 0x54, 0x0B, 0xF7, 0xB1, 0xCD, 0xB6, 0x06, 0xE8, 0x57, 0x23, 0x3E, 0x0E}, 32)) {
	...
	bytes = CBNewByteArrayWithDataCopy((unsigned char [58]){0x1, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xff, 0xff, 0xff, 0xff, 0x7, 0x4, 0xff, 0xff, 0x0, 0x1d, 0x1, 0x4, 0xff, 0xff, 0xff, 0xff, 0x0, 0x0, 0x0, 0x0, 0x0}, 58);
	tx = CBNewTransactionFromData(bytes);
	if (CBTransactionDeserialise(tx) != CB_DESERIALISE_ERROR) {
		printf("TX ZERO INPUTS DESERAILISE FAIL\n");
		return 1;
	...
	script = CBNewScriptMultisigOutput((unsigned char *[2]){keyPairs[0]->pubkey.key, keyPairs[1]->pubkey.key}, 2, 2);
